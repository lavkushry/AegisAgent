...
def _rebind_edited_args(
    sig: inspect.Signature,
    parameters: dict,
    edited_params: dict,
) -> Tuple[list, dict]:
    """Re-bind function arguments from reviewer-edited parameters.

    For each parameter in *sig*, prefer the value from *edited_params*; fall
    back to the original *parameters* if the key is absent from the edits.
    Returns (positional_args, keyword_args) ready to splat into ``func(*a, **kw)``.
    """
    new_args: list = []
    new_kwargs: dict = {}
...
def protect_tool(
    ...
    *,
    ...
    Poll behaviour (exponential backoff):
        poll_interval     – initial sleep duration in seconds (default 1.0).
        poll_backoff      – multiplier applied after each poll (default 1.5).
        poll_max_interval – cap on the sleep duration in seconds (default 10.0).
        poll_timeout      – wall-clock deadline for the whole approval wait
                            in seconds (default 300.0 = 5 minutes).
    """
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        def wrapper(*args, **kwargs):
            ...
            if decision == "allow":
                ...
                if not approval:
                    raise PermissionError(
                        f"Action '{tool}.{action}' requires approval but no approval "
                        "info was returned. Failing closed."
                    )
                ...
                # Poll gateway with exponential backoff, bounded by a wall-clock deadline.
                ...
                while time.monotonic() < deadline:
                    ...
                    if status == "APPROVED":
                        ...
                        new_args, new_kwargs = _rebind_edited_args(
                            sig, parameters, edited_params
                        )
                        return func(*new_args, **new_kwargs)

                raise TimeoutError(
                    f"Action '{tool}.{action}' approval request timed out after "
...
def async_protect_tool(
    ...
    *,
    ...
    """Async variant of :func:`protect_tool` for ``async def`` tool functions.

    Sync client calls (``authorize``, ``get_approval_status``,
    ``consume_approval``) are dispatched via ``asyncio.to_thread`` so the
    event loop is never blocked.  Polling uses ``asyncio.sleep`` with the same
    configurable exponential-backoff parameters as the sync decorator.

    All fail-closed guarantees are identical to ``protect_tool``:
    - deny → PermissionError, tool NOT executed.
    - hash mismatch at any phase → PermissionError.
    - expiry (initial or mid-poll) → PermissionError.
    - single-use consume failure → PermissionError.
    - REJECTED → PermissionError.
    - EDITED → re-bind reviewer params, then await func.
    - unknown decision → PermissionError.
    - deadline exceeded → TimeoutError.
    - allow → await func, result returned.
    """
