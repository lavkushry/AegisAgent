-- Phase 7.1 (prompt/model capture): lineage-only records for prompt and
-- model-call activity. Stores a hash + a caller-redacted preview — never a
-- raw prompt, request, or response body. `event_id` is the producer-assigned
-- id; the `(tenant_id, event_id)` unique index makes ingest idempotent (a
-- replayed/retried event is a no-op). Tenant-scoped.
