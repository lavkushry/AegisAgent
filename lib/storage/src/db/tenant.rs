...
pub async fn list_api_keys_cursor(
    pool: &DbPool,
    tenant_id: &str,
    limit: i64,
    offset: i64,
    cursor: Option<i64>,
) -> Result<(Vec<ApiKeyRecord>, Option<i64>), sqlx::Error> {
    let limit = limit.clamp(1, crate::db::SOC_MAX_LIMIT);
    let query = "SELECT *, rowid FROM api_keys
         WHERE tenant_id = ?
...
mod tests {
    ...
    /// #1142: `list_api_keys_cursor` returns a `next_cursor` when more rows
    /// exist beyond the requested page.
    #[tokio::test]
    async fn list_api_keys_cursor_paginates_and_sets_next_cursor() {
        let pool = setup_pool("api_keys_cursor_paginate").await;
        let tenant_id = "tenant_api_keys_cursor_paginate".to_string();
        register_tenant(&pool, &tenant_id, "Cursor Test Tenant", "developer")
            .await
            .unwrap();
        for i in 0..3 {
            create_api_key(&pool, &tenant_id, &format!("key-{i}"))
                .await
                .unwrap();
        }

        let (page, next_cursor) = list_api_keys_cursor(&pool, &tenant_id, 2, 0, None)
            .await
            .unwrap();
    ...
    /// next page exists.
    #[tokio::test]
    async fn list_api_keys_cursor_no_false_next_cursor_at_exact_boundary() {
        let pool = setup_pool("api_keys_cursor_boundary").await;
        let tenant_id = "tenant_api_keys_cursor_boundary".to_string();
        register_tenant(&pool, &tenant_id, "Boundary Test Tenant", "developer")
            .await
            .unwrap();
        create_api_key(&pool, &tenant_id, "key-a").await.unwrap();

        let (page, next_cursor) = list_api_keys_cursor(&pool, &tenant_id, 1, 0, None)
            .await
            .unwrap();
