...
| `lib/decision` | Full pipeline `run_authorize_pipeline` (admit→preflight→guard→metadata→evaluate) + `DecisionRuntime` ports; gateway implements ports | Decision seam / authorize orchestration | Hot path still uses async/storage via host ports (transitional v1) | Reactor snapshots and control-store cutover in later weeks |
