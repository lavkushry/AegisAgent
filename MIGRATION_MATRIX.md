...
| `src/src/routes/authorize.rs` | Thin `authorize_action_impl`: context → `run_authorize_pipeline` → wire map; route tests in `authorize_tests.rs`; wire helpers/tests in `authorize_service(_tests).rs` + equality corpus | REST adapter | Large route test module still co-located under routes | Optionally move route tests to `tests/` / integration crate |
| `src/src/grpc.rs` | **Typed via AuthorizedOutcome** for control/MCP/query/close; **storage-direct** SOC RPCs map DB errors through `aegis_error_to_tonic` | gRPC adapter | Storage-direct SOC RPCs still thicker than control path | Continue typed service extraction for remaining SOC RPCs |
| `lib/decision` | Full pipeline `run_authorize_pipeline` + `DecisionRuntime` ports; stage units + pipeline e2e in `*_tests.rs` path modules; gateway implements ports | Decision seam / authorize orchestration | Hot path still uses async/storage via host ports (transitional v1) | Reactor snapshots and control-store cutover in later weeks |
...
| Disruptor/SPSC event fabric | Tokio bounded MPSC remains `current`; unwired `lib/event` (Proposed ADR-0006..0010) adds 64-byte-separated cursors, Acquire/Release sequences, 32-byte descriptors, bounded closure/drop, cancelable producer permits, commit-delayed consumer claims, a safe sealed-page oracle, an append-only published prefix, `VolatileAdmissionChannel` composition, and `RotatingAdmissionChannel<N,P>` (fixed pool of `P` pages, exclusive in-place rebind, handoff ring, typed `PageQuotaExhausted`, Loom seam + native stress, diagnostic criterion bench, LLD §5 linearization). Composites validate before reservation, publish the page before the ring, withhold capacity until a validated frame lease commits, and distinguish clean/faulted/orphaned-prefix termination. | The production fabric remains `target`, neither `shadow` nor `qualified`, carries no protected evidence, and has no published performance result. Blockers are formal ADR acceptance/security review, green hosted sanitizer artifacts, real UBSan support, authenticated registry lookup, WAL durability/replay, NUMA-owner reclamation, priority lanes, production shadow wiring, release-artifact rollback, and qualification. |
