...
| `src/src/routes/authorize.rs` | JSON parse at `authorize_action_impl`; `tokio::join!`; Cedar and storage orchestration in handler | Full authorization transaction | Adapter is business layer; repeated row lookups/writes dominate latency | Extract `aegis-decision::AuthorizeService`; handler becomes parse → call → map |
| `src/src/grpc.rs` | Most gRPC methods still invoke REST handlers and buffer Axum bodies; **`authorize` is typed** via `authorize_service` → `AuthorizedOutcome` → `outcome_to_tonic` (no adapter-side Axum body buffer, no forged loopback) | gRPC adapter | Remaining RPCs still Protobuf → REST model → JSON body → protobuf | Finish typed service calls for remaining RPCs; in-place Result returns inside `authorize_action_impl` eliminate internal Response decode |
