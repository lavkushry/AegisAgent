...
| `src/src/grpc.rs` | Most gRPC methods still invoke REST handlers and buffer Axum bodies; **`authorize` is typed** via `authorize_service` (Phase D, no forged loopback / no Status::internal collapse) | gRPC adapter | Remaining RPCs still Protobuf → REST model → JSON body → protobuf | Finish typed service calls for remaining RPCs; Result-typed `authorize_core` removes residual Response buffer on authorize |
