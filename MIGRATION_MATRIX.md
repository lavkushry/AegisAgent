...
| `src/src/grpc.rs` | **Typed:** authorize, approve, register_agent, create_tenant, register_mcp_server, discover_mcp_tools. **Still bridged:** SOC family (query/close use StatusError→tonic mapping) | gRPC adapter | Residual SOC RPCs still materialize Axum responses | Finish typed SOC service calls; later `aegis-decision` extraction |
