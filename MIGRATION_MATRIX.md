...
| `src/src/grpc.rs` | **`authorize` + `approve` fully typed** (`AuthorizedOutcome` → tonic); other RPCs still REST-bridge but map errors via `axum_response_to_tonic_json` (no Status::internal collapse) | gRPC adapter | Remaining RPCs still materialize Axum responses | Finish typed service calls for remaining RPCs; later `aegis-decision` extraction |
