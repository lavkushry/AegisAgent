...
| `src/src/grpc.rs` | Most gRPC methods still invoke REST handlers and buffer Axum bodies; **`authorize` is fully typed** (`authorize_action_impl` → `AuthorizedOutcome` → `outcome_to_tonic`; no Axum Response on the authorize path) | gRPC adapter | Remaining RPCs still Protobuf → REST model → JSON body → protobuf | Finish typed service calls for remaining RPCs; later `aegis-decision` extraction |
