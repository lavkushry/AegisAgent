...
| `src/src/grpc.rs` | **Typed via AuthorizedOutcome** for control/MCP/query/close; **storage-direct** SOC RPCs map DB errors through `aegis_error_to_tonic` | gRPC adapter | Evaluation still in gateway | Implement `aegis_decision::AuthorizeService` in `lib/decision` and thin adapters |
| `lib/decision` | **Introduced:** protocol-neutral `AuthorizeContext` / `AuthCredential` / `Transport` + `AuthorizeService` trait (no Axum/tonic) | Decision seam | Evaluation still in `authorize_action_impl` | Move evaluation behind the trait |
