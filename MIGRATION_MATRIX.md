...
| `src/src/routes/authorize.rs` | Thin `authorize_action_impl`: context → `run_authorize_pipeline` → wire map; tests in `authorize_tests.rs` | REST adapter | Large test module still co-located under routes | Optionally move tests to `tests/` / integration crate |
...
| `lib/decision` | Full pipeline `run_authorize_pipeline` (admit→preflight→guard→metadata→evaluate) + `DecisionRuntime` ports; gateway implements ports; pipeline e2e in `pipeline_tests.rs` | Decision seam / authorize orchestration | Hot path still uses async/storage via host ports (transitional v1) | Reactor snapshots and control-store cutover in later weeks |
