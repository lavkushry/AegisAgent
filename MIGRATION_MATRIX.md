...
| `lib/decision` | Context types + `AuthorizeService` trait; gateway `GatewayAuthorizeService` implements the trait | Decision seam | Evaluation body still in `authorize_action_impl` | Move evaluation into `aegis-decision` |
