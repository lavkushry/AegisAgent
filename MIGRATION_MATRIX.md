...
| `src/src/routes/authorize.rs` | Thin `authorize_action_impl`: context from headers → `run_authorize_pipeline` → wire map | REST adapter | Large inline test module still in routes | Keep adapter thin; optionally relocate tests to integration crate |
