...
| `src/src/grpc.rs` | gRPC methods invoke REST handlers, collect Axum bodies, then parse JSON | gRPC adapter | Protobuf → REST model → JSON body → protobuf response creates avoidable materialization | Replace with direct typed service calls; delete JSON bridge after RPC parity |
