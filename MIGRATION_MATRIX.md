...
| `src/src/grpc.rs` | **Typed:** authorize, approve, register_agent, create_tenant, register_mcp_server (`AuthorizedOutcome` → tonic). **Still bridged:** discover_mcp_tools, SOC family, … | gRPC adapter | Residual RPCs still materialize Axum responses | Finish typed service calls for remaining RPCs; later `aegis-decision` extraction |
