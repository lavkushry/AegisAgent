...
| `src/src/grpc.rs` | **Typed via AuthorizedOutcome:** authorize, approve, register_agent, create_tenant, register/discover MCP, soc_query, close_incident. **Storage-direct:** playbooks, alerts lists, contact points, dashboards, semantic search | gRPC adapter | Storage-direct methods still use Status::internal for DB errors | Map storage errors through StatusError; later `aegis-decision` extraction |
