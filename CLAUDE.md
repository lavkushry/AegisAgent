...
**Rust gateway — 522 tests, verified on `main`:**
...
- **Evidence graph query optimization** (#1316): `add_decision_subgraph` no longer issues up to 2 sequential unindexed approval/receipt lookups per decision (up to 100 queries for a 50-decision agent graph). `db::list_approvals_by_decision_ids`/`db::list_action_receipts_by_decision_ids` batch-fetch all approvals/receipts for a decision set in one `tenant_id = ? AND decision_id IN (...)` query each; `get_graph_for_run`/`get_graph_for_incident`/`get_graph_for_agent` prefetch both maps once before building nodes. New composite indexes `idx_approvals_tenant_decision`/`idx_action_receipts_tenant_decision` (migration `0015_graph_query_indexes.sql`) match the query shape. Criterion benchmark `evidence_graph_benchmark` (`get_graph_for_agent`, 50 seeded decisions each with an approval + receipt) measures ~6.3ms mean, well under the issue's <100ms target. 4 new gateway tests.
...
cargo test   --manifest-path gateway/Cargo.toml        # 522 tests
