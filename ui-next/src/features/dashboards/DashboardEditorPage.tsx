...
import { probeGatewayEndpoint } from "@/lib/http/client";
...
export function DashboardEditorPage() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<SocDashboardRecord | null>(
    null,
  );

  const { data: backendReady } = useQuery({
    queryKey: ["dashboardEditorSupport", gatewayUrl, activeTenant, bearerToken],
    queryFn: ({ signal }) =>
      probeGatewayEndpoint({ ...apiOpts, signal }, "/v1/soc/dashboards"),
    enabled: tenantReady && Boolean(bearerToken),
    retry: false,
  });
  ...
  const {
    ...
    queryKey: ["socDashboards", gatewayUrl, activeTenant, bearerToken],
    queryFn: ({ signal }) => listSocDashboards({ ...apiOpts, signal }),
    enabled: tenantReady && Boolean(bearerToken) && backendReady === true,
    retry: false,
  });
  ...
  const copyFromSystem = (uid: string) => {
    ...
    setValidationMessage(
      `Copied system dashboard '${uid}' — assign a tenant uid before saving.`,
    );
  };
  ...
  if (!tenantReady) {
    return <TenantGate title="Select a tenant for Dashboards" />;
  }
    ...
    <div className="space-y-4">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-[11px] text-[var(--text-muted)]">
            Edit tenant dashboards using the same{" "}
            <code className="font-mono">DashboardSchema</code> as system pages.
      ...
      {backendReady === false ? (
        <p className="rounded border border-[var(--border-default)] bg-[var(--surface-panel)] px-3 py-2 text-xs text-[var(--text-secondary)]">
          Gateway endpoint{" "}
          <code className="font-mono">/v1/soc/dashboards</code> is unavailable
          on this tenant. Upgrade the gateway to persist dashboards; you can
          still validate and export JSON locally.
        </p>
      ) : null}
      ...
      <div className="grid gap-4 lg:grid-cols-[240px_minmax(0,1fr)]">
          ...
          <section>
            ...
            {savedDashboards.length === 0 ? (
              <p className="text-xs text-[var(--text-muted)]">
                No tenant dashboards yet.
              </p>
            ) : (
              <ul className="space-y-1">
                {savedDashboards.map((record) => (
                  <li key={record.id || record.uid}>
                    <button
                      type="button"
                      className={`w-full rounded px-2 py-1 text-left text-xs ${
                        selectedUid === record.uid
                          ? "bg-[var(--brand)] text-[var(--text-on-brand)]"
                          : "hover:bg-[var(--surface-app)]"
                      }`}
        ...
        <div className="space-y-3">
          <div className="flex flex-wrap gap-2">
            ...
            {selectedUid ? (
              <button
                ...
                onClick={() => {
                  const record = savedDashboards.find(
                    (d) => d.uid === selectedUid,
                  );
          ...
          {showPreview && !previewSchema ? (
            <p className="text-xs text-[var(--sev-critical)]">
      ...
      <ConfirmDialog
        ...
        target={
          deleteTarget ? `${deleteTarget.title} (${deleteTarget.uid})` : "—"
        }
        confirmLabel="Delete"
        onConfirm={() => {
