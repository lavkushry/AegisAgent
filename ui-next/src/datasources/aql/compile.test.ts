...
describe("AQL compiler", () => {
  test("compiles AND-only filters to flat gateway parameters", () => {
  ...
  test("maps aliases and time ranges into gateway filter keys", () => {
  ...
  test("compiles aggregate metadata for structured /v1/soc/query requests", () => {
  ...
  test("fails closed on OR execution instead of silently broadening", () => {
    expect(() => aqlToCompiledRequest("agent_id:a OR agent_id:b", "decision")).toThrow(AqlCompileError);
  });

  test("preserves legacy compileAqlString shape for decisions fallback", () => {
    expect(
