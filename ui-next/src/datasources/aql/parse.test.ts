...
describe("AQL parser", () => {
  test("compiles supported field filters into typed gateway parameters", () => {
  ...
  test("compiles investigation identifiers and evidence hashes as typed filters", () => {
    const query = parseAql(
  ...
  test("compiles ASE fields and keeps free terms in q", () => {
  ...
  test("returns an empty filter for whitespace", () => {
    expect(parseAql("   ")).toEqual({ filter: { kind: "bool", op: "and", children: [] } });
  });

  test("respects OR precedence below AND", () => {
  ...
  test("parses grouped boolean filters", () => {
  ...
  test("parses time ranges on @time", () => {
  ...
  test("rejects unknown fields with positional errors", () => {
  ...
  test("rejects unsupported intervals and operators in aggregates", () => {
  ...
  test("bounds input length and injection-like strings without broadening fields", () => {
  ...
  test("round-trips valid queries through serialize and URL encoding", () => {
