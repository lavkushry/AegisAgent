...
/// Default interval between threat hunt sweeps (#1395).
pub const DEFAULT_THREAT_HUNT_POLL_INTERVAL_SECS: u64 = 3600;
...
/// Default batch size per tenant per investigation sweep tick.
pub const DEFAULT_INVESTIGATION_BATCH_LIMIT: i64 = 20;
...
/// #1395: periodic threat hunt sweep for anomalous agent behavior patterns.
/// Leader-gated like other maintenance jobs.
pub async fn run_threat_hunt_job(
    pool: DbPool,
    interval_secs: u64,
    batch_limit: i64,
    is_leader: Arc<AtomicBool>,
    ...
    loop {
        interval.tick().await;
        if !is_leader.load(Ordering::Relaxed) {
            debug!("threat hunt job: standby (not leader)");
            continue;
        }
        let tenant_ids = match db::list_all_tenant_ids(&pool).await {
            Ok(ids) => ids,
            Err(e) => {
                error!("threat hunt job: failed to list tenants: {:?}", e);
                continue;
            }
        };
        for tenant_id in tenant_ids {
            match crate::threat_hunter::generate_threat_hunt_findings_for_tenant(
                &pool,
                &tenant_id,
                batch_limit,
            )
            .await
            {
                Ok(0) => {}
                Ok(n) => info!(
                    "threat hunt job: created {} finding(s) (tenant={})",
                    n, tenant_id
                ),
...
/// #1392: periodic investigation playbook sweep for open incidents.
/// Leader-gated like other maintenance jobs.
pub async fn run_investigation_job(
    pool: DbPool,
    interval_secs: u64,
    batch_limit: i64,
    is_leader: Arc<AtomicBool>,
    ...
    loop {
        interval.tick().await;
        if !is_leader.load(Ordering::Relaxed) {
            debug!("investigation job: standby (not leader)");
            continue;
        }
        let tenant_ids = match db::list_all_tenant_ids(&pool).await {
            Ok(ids) => ids,
            Err(e) => {
                error!("investigation job: failed to list tenants: {:?}", e);
                continue;
            }
        };
        for tenant_id in tenant_ids {
            match crate::investigation::generate_investigations_for_tenant(
                &pool,
                &tenant_id,
                batch_limit,
            )
            .await
            {
                Ok(0) => {}
                Ok(n) => info!(
                    "investigation job: created {} playbook(s) (tenant={})",
                    n, tenant_id
                ),
                Err(e) => error!("investigation job failed for tenant {}: {:?}", tenant_id, e),
            }
        }
    }
}
