...
//! - This module: adapter-facing re-exports, maps [`DecisionOutcome`] ↔
//!   [`AuthorizedOutcome`], REST/gRPC status helpers, and
//!   [`GatewayAuthorizeService`]. No gRPC→Axum JSON bridge remains.
...
/// Build the HeaderMap for residual gateway code that still needs transport
/// headers (OTel parent propagation from typed context). Adapters prefer
...
#[cfg(test)]
#[path = "authorize_service_tests.rs"]
mod tests;
