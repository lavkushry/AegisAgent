//! Integration/unit tests for authorize routes.
//!
//! Loaded as `routes::authorize::tests` via `#[path]` so production
//! `authorize.rs` stays a thin adapter over `run_authorize_pipeline`.
...
async fn authorize_action_unaffected_by_admission_webhook_pass() {
    ...
    let (state_with_webhook, tenant_id, agent_token) =
        setup_state_with_admission_webhook("admission_pass", &format!("http://{addr}/admit"), true)
            .await;
...
async fn authorize_emits_verifiable_receipt() {
    ...
    // for it to land before querying for it.
    state
        .deferred_write_tracker
        .drain(std::time::Duration::from_secs(5))
        .await;
...
async fn authorize_records_agent_risk_score() {
    ...
    // #1512: the risk-score sample is now a deferred background write —
...
/// #1512: the risk-score sample and the verifiable receipt are both
/// written off the `/v1/authorize` hot path via the same
/// [`DeferredWriteTracker`] — the decision/audit row (synchronous) is
/// visible immediately, but the risk-score and receipt rows land only
/// once the tracker has drained. One `drain()` call must be sufficient
/// for both, since they're spawned from the same decision.
#[tokio::test]
async fn authorize_defers_risk_score_and_receipt_writes_until_drained() {
    let (state, tenant_id, agent_token) = setup_state("authorize_defers_writes").await;
    register_ship_action(&state, &tenant_id, "low").await;
    ...
    // A single drain() call is sufficient for both deferred writes
    // spawned by that one decision.
    let drained = state
        .deferred_write_tracker
        .drain(std::time::Duration::from_secs(5))
        .await;
    ...
    assert_eq!(
        scores.len(),
        1,
        "risk-score sample should have landed after drain"
    );

    let (receipt_count,): (i64,) = sqlx::query_as(
        "SELECT COUNT(*) FROM action_receipts WHERE tenant_id = ? AND decision_id = ?",
    )
    .bind(tenant_id.as_str())
    .bind(&decision.id)
    .fetch_one(state.storage.get_pool().sqlite_pool())
    ...
    assert_eq!(
        receipt_count, 1,
        "receipt should have landed after the same drain"
    );
}
...
async fn insert_audit_event_persists_microsecond_created_at_for_chronological_ordering() {
    let (state, tenant_id, _agent_token) = setup_state("audit_event_microsecond_created_at").await;
...
async fn authorize_is_idempotent_for_repeated_request_id() {
    ...
    let second = call_authorize(state.clone(), &tenant_id, &agent_token, approval_request).await;
...
async fn request_signing_valid_signature_allows() {
    ...
    let (state, tenant_id, agent_token) = setup_state_with_signing_key("signing_valid", key).await;
...
async fn replay_consume_emits_tamper_receipt() {
    ...
    // #1512: the original decision's receipt write is now deferred —
    // wait for it to land so it's the chain head the tamper-attempt
    // receipt below chains onto (otherwise the tamper receipt could win
    // the race and become the head itself, with an empty
    // `prev_receipt_hash`).
    state
        .deferred_write_tracker
        .drain(std::time::Duration::from_secs(5))
    ...
    let (before,): (i64,) =
        sqlx::query_as("SELECT COUNT(*) FROM action_receipts WHERE tenant_id = ? AND decision = ?")
            .bind(tenant_id.as_str())
...
async fn authorize_action_denies_frozen_and_revoked_agent() {
    ...
    let request = mcp_authorize_request("filesystem", "read_file");
    let allowed = call_authorize(state.clone(), &tenant_id, &agent_token, request.clone()).await;
...
async fn force_approval_agent_downgrades_allow_to_require_approval() {
    ...
    let downgraded = call_authorize(state.clone(), &tenant_id, &agent_token, request.clone()).await;
...
async fn authorize_denies_when_tool_permission_revoked_even_with_request_id() {
    let (state, tenant_id, agent_token) = setup_state("perm_denied_concurrent_idempotency").await;
    let agent_id = state
        .storage
        .get_agent_by_token(&tenant_id, &agent_token)
        .await
...
async fn authorize_denies_high_risk_action_when_db_locked_then_recovers() {
    ...
    // busy_timeout(50ms) → any concurrent writer lock still surfaces as
    // SQLITE_BUSY quickly (test stays fast and deterministic), but
    // tolerates the #1512 deferred risk-score/receipt writes from the
    // FIRST call below still finishing on a separate connection (each
    // retries internally for up to ~7ms) when the SECOND call's own
    // synchronous writes land. A 0ms timeout conflated "externally
    // locked the whole test" with "this request's own deferred writes
    // briefly overlapping" — both used to surface as SQLITE_BUSY
    // identically, but only the former should deny.
