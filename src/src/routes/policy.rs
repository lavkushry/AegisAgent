...
pub async fn list_policy_audit_log(
    ...
    match state
        .storage
        .list_policy_audit_log_cursor(&tenant_id, limit, offset, cursor)
        .await
    {
        Ok((entries, next_cursor)) => super::paginated_response(&entries, next_cursor),
        Err(e) => {
...
"#;
    ...
    #[tokio::test]
    async fn list_policy_audit_log_route_sets_next_cursor_header() {
        let (state, tenant_id, _) = setup_state("list_policy_audit_log_cursor_header").await;
        for i in 0..2 {
            let payload = CreatePolicyRequest {
                policy_key: format!("audit-cursor-policy-{i}"),
                name: format!("Audit Cursor Policy {i}"),
                body: "permit (principal, action, resource);".to_string(),
            };
            let _ = create_policy(
                State(state.clone()),
        ...
        let response = list_policy_audit_log(
            State(state.clone()),
            TenantId(tenant_id.clone()),
            axum::extract::RawQuery(Some("limit=1".to_string())),
        )
        ...
        let next_cursor = response
            .headers()
            .get("x-next-cursor")
            .expect("a second audit-log entry exists beyond the page")
            .to_str()
            .unwrap()
            .to_string();
        let body = to_bytes(response.into_body(), usize::MAX).await.unwrap();
        let page: Vec<PolicyAuditLogRecord> = serde_json::from_slice(&body).unwrap();
        assert_eq!(page.len(), 1);

        let response2 = list_policy_audit_log(
            State(state),
            TenantId(tenant_id),
            axum::extract::RawQuery(Some(format!("limit=1&cursor={next_cursor}"))),
        )
        ...
        let body2 = to_bytes(response2.into_body(), usize::MAX).await.unwrap();
        let page2: Vec<PolicyAuditLogRecord> = serde_json::from_slice(&body2).unwrap();
        assert_eq!(page2.len(), 1);
    }
