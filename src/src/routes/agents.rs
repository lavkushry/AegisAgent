...
mod tests {
    ...
    async fn test_patch_agent_route() {
        ...
        let db_agent = state
            .storage
            .get_agent_by_id(&tenant_id, "patch_agent_test_id")
            .await
            .unwrap()
            .unwrap();
    ...
    async fn test_patch_agent_route_binds_mtls_cn() {
        ...
        let resolved = state
            .storage
            .get_agent_by_mtls_cn(&tenant_id, "agent-cert-cn-123")
            .await
            .unwrap()
            .unwrap();
    ...
    async fn revoke_agent_tool_permission_route_writes_admin_action_audit_event() {
        ...
        state
            .storage
            .grant_agent_tool_permission(&tenant_id, &agent.id, "github")
            .await
            .unwrap();
    ...
    async fn restore_agent_reactivates_quarantined_agent() {
        ...
        let resp_before = authorize_action_impl(
            state.clone(),
    ...
    async fn rotate_agent_token_invalidates_old_token_and_issues_new_one() {
        ...
        // New token works.
        let req2 = mcp_authorize_request("filesystem", "read_file");
        let resp_new = authorize_action_impl(
            state.clone(),
    ...
    async fn agent_lifecycle_columns_are_populated_and_cleared() {
        ...
        assert!(state
            .storage
            .set_agent_status(&tenant_id, &agent_id, "active")
            .await
            .unwrap());
        let agent = state
            .storage
    ...
    async fn test_agent_token_is_hashed_in_db() {
        ...
        let stored_agent = state
            .storage
            .get_agent_by_key(&tenant_id, "hash-agent")
            .await
            .unwrap()
        ...
        let resolved = state
            .storage
            .get_agent_by_token(&tenant_id, &cleartext_token)
            .await
            .unwrap();
    ...
    async fn register_tool_creates_skill_with_actions() {
        ...
        let action = state
            .storage
            .get_skill_action(&tenant_id, "deployer", "ship")
            .await
            .unwrap()
