...
mod tests {
    ...
    async fn evidence_graph_has_no_orphan_nodes_or_dangling_edges() {
        ...
        let mut receipt = unsigned_receipt_template(&tenant_id);
        receipt.decision_id = Some(decision.id.clone());
        state
            .storage
    ...
    async fn test_evidence_graph_cross_tenant_isolation_stress() {
        ...
        let (state, _base_tenant_id, _base_agent_token) =
            setup_state("graph_cross_tenant_stress").await;
        let tenant_a = "tenant_a".to_string();
        let tenant_b = "tenant_b".to_string();
        for (tenant_id, name) in [(&tenant_a, "Tenant A"), (&tenant_b, "Tenant B")] {
            db::register_tenant(state.storage.get_pool(), tenant_id, name, "developer")
                .await
                .unwrap();
        }
