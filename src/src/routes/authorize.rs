...
pub async fn authorize_action_impl(
    ...
    let payload = guarded.request;
    let dry_run = guarded.dry_run;
    let root_trust_level = guarded.root_trust_level;
    let agent = guarded.agent;
    let is_mtls = guarded.used_mtls;
    let tenant_id = agent.tenant_id.clone();
    let agent_id = agent.id.clone();
    let normalized_tool = guarded.normalized_tool;
    let normalized_action = guarded.normalized_action;
    let action_hash = guarded.action_hash;
