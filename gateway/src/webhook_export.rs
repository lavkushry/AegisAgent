...
use crate::models::WebhookSubscriptionRecord;
...
pub async fn dispatch(pool: &SqlitePool, client: &reqwest::Client, payload: &WebhookExportPayload) {
    let subscriptions = match crate::db::list_matching_webhook_subscriptions(
        pool,
        &payload.tenant_id,
        &payload.event_type,
    )
...
async fn deliver_with_retry(
    ...
    if let Err(e) = crate::db::record_webhook_delivery_result(
        pool,
...
mod tests {
    ...
    async fn dispatch_delivers_signed_json_payload_and_records_success() {
        ...
        let updated = crate::db::get_webhook_subscription(&pool, "tenant_1", &subscription.id)
            .await
            .unwrap()
            .unwrap();
