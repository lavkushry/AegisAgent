...
pub async fn maybe_escalate_agent_risk_tier(
    ...
    let config = crate::db::get_risk_escalation_config(pool, tenant_id).await?;
    let since: DateTime<Utc> = Utc::now() - Duration::minutes(config.window_minutes);
    let denial_count = crate::db::count_recent_denials(pool, tenant_id, agent_id, since).await?;
    ...
    crate::db::update_agent_risk_tier(pool, tenant_id, agent_id, new_tier).await?;
