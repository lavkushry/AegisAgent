...
def merge_pull_request(repo: str, pr_number: int, base_branch: str = "main"):
    logger.info(f"🚀 Executing merge_pull_request on repo={repo}, PR={pr_number}, branch={base_branch}")
...
def simulate_human_approval(approval_id: str, action: str = "approve"):
    ...
    headers = {
        "Authorization": "Bearer tenant_123",
    ...
    try:
        logger.info(f"👤 Reviewer: Sending {action.upper()} request for approval ID {approval_id}...")
...
def run_test_scenario():
    ...
    if not client.register_agent(agent_name="Production Coding Agent", owner_team="platform"):
        logger.error("Failed to register agent. Make sure the gateway is running on port 8080.")
        return
    ...
    try:
        resp = requests.post(f"{GATEWAY_URL}/v1/tools", json=tool_payload, headers=headers, timeout=5)
    ...
    logger.info("Context trust level set to: 'untrusted_external' (e.g. following public GitHub issue input).")
    try:
        ...
        logger.info(f"✅ Test Pass: Action successfully blocked by AegisAgent. Error: {e}")
    ...
    logger.info("Context trust level set to: 'semi_trusted_customer' (e.g. following customer comment).")
    ...
    def approval_helper():
        ...
        try:
            resp = requests.get(f"{GATEWAY_URL}/v1/audit/events", headers=headers, timeout=5)
            if resp.status_code == 200:
                events = resp.json()
    ...
    try:
        ...
        logger.info(f"✅ Test Pass: Action successfully approved and executed! Result: {result}")
    except Exception as e:
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            for event in reversed(events[:5]): # show last 5 events
                logger.info(f"  - [{event.get('created_at')}] Event: {event.get('event_type')} for action: {event.get('action')}")
        else:
