...
import { parseAndValidateDashboardJson, validateDashboardSchema } from "./validate";
...
describe("dashboard schema validation", () => {
  ...
  it("rejects reserved system uids", () => {
  ...
  it("rejects unknown panel types", () => {
    ...
    const layout = schema.layout as Array<{ panels: Array<{ panel: { type: string } }> }>;
    layout[0].panels[0].panel.type = "iframe";
    const result = validateDashboardSchema(schema as never);
    expect(result?.kind).toBe("schema");
  });

  it("round-trips a system dashboard export", () => {
    const exported = JSON.stringify(overviewDashboard);
    const copy = JSON.parse(exported) as typeof overviewDashboard & { uid: string; title: string };
