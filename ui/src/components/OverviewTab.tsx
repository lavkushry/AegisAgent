...
export default function OverviewTab() {
  ...
  if (summaryError) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center bg-[#111827] border border-[#334155] rounded-xl p-8">
        <AlertTriangle className="text-red-500 mb-4" size={48} />
        <h3 className="text-lg font-bold text-[#e2e8f0]">Gateway Connection Error</h3>
        <p className="text-sm text-[#94a3b8] mt-2 max-w-md">
          Could not fetch data from the AegisAgent gateway at <code className="text-indigo-400">{gatewayUrl}</code>. Ensure the gateway service is running and your bearer token is valid.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="panel-card flex flex-col justify-between h-28">
          ...
          <div className="text-3xl font-extrabold text-[#e2e8f0]">
        ...
        <div className="panel-card flex flex-col justify-between h-28 border-amber-500/30">
          ...
          <div className="text-[10px] text-[#94a3b8]">Requires human validation</div>
        </div>
        ...
        <div className="panel-card flex flex-col justify-between h-28 border-rose-500/30">
          ...
          <div className="text-[10px] text-[#94a3b8]">Active security events</div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="panel-card flex flex-col">
          <h3 className="text-xs font-bold text-[#94a3b8] uppercase tracking-wider mb-4 flex items-center gap-1">
            <Flame size={14} className="text-rose-500" /> Top Risky Agents
          </h3>
          <div className="flex-1 overflow-y-auto max-h-[250px] custom-scrollbar">
            {isScoreboardLoading ? (
              <p className="text-xs text-[#64748b] text-center py-8">Loading scoreboard...</p>
            ) : !scoreboard || scoreboard.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-center">
                <UserCheck className="text-green-500 mb-2" size={32} />
                <p className="text-xs text-[#94a3b8]">All agents running safely</p>
              </div>
            ) : (
              <table className="w-full text-left text-xs">
                <thead>
                  <tr className="border-b border-[#334155]">
                    <th className="py-2 text-[#64748b] font-medium">AGENT</th>
                    <th className="py-2 text-[#64748b] font-medium text-right">RISK SCORE</th>
                    <th className="py-2 text-[#64748b] font-medium text-right">TREND</th>
                  </tr>
                </thead>
                <tbody>
                  {scoreboard.slice(0, 5).map((row: any, idx: number) => (
                    <tr key={idx} className="border-b border-[#1f2937] hover:bg-[#1f2937]/30">
                      <td className="py-3 font-mono text-indigo-400">{row.agent_id}</td>
                      <td className="py-3 text-right font-bold text-rose-500">
                        {row.avg_risk_score}
                      </td>
                      <td className={`py-3 text-right font-medium ${row.trend === "rising" ? "text-red-500" : row.trend === "falling" ? "text-green-500" : "text-[#64748b]"}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="panel-card">
          <h3 className="text-xs font-bold text-[#94a3b8] uppercase tracking-wider mb-4 flex items-center gap-1.5">
            <ShieldAlert size={14} className="text-rose-500" /> Recent Security Incidents
          </h3>
          <div className="space-y-3">
            {!recentIncidents || recentIncidents.length === 0 ? (
              <p className="text-xs text-[#64748b] text-center py-6">No incidents recorded.</p>
            ) : (
              recentIncidents.slice(0, 3).map((inc: any, idx: number) => (
                <div key={idx} className="p-3 bg-[#0f172a] border border-rose-500/20 rounded-lg hover:border-rose-500/40 transition-colors">
                  <div className="flex justify-between items-start gap-2">
                    <strong className="text-xs font-semibold text-rose-500">{inc.kind}</strong>
                    <span className="text-[10px] text-[#64748b] font-mono">{inc.id.slice(-8)}</span>
                  </div>
                  <p className="text-xs text-[#e2e8f0] mt-1">{inc.summary}</p>
                  <div className="flex justify-between items-center mt-2 text-[10px] text-[#94a3b8]">
                    <span>Agent: <code className="text-indigo-400">{inc.agent_id}</code></span>
        ...
        <div className="panel-card">
          ...
          <div className="space-y-3">
            {!recentAlerts || recentAlerts.length === 0 ? (
              <p className="text-xs text-[#64748b] text-center py-6">No alerts triggered.</p>
            ) : (
              recentAlerts.slice(0, 3).map((alert: any, idx: number) => (
                <div key={idx} className="p-3 bg-[#0f172a] border border-[#334155] rounded-lg hover:border-indigo-500/30 transition-colors">
                  <div className="flex justify-between items-start gap-2">
                    <strong className="text-xs font-semibold text-amber-500">{alert.rule}</strong>
                    <span className="text-[10px] text-[#64748b] font-mono">{alert.id.slice(-8)}</span>
                  </div>
                  <p className="text-xs text-[#e2e8f0] mt-1">{alert.summary}</p>
                  <div className="flex justify-between items-center mt-2 text-[10px] text-[#94a3b8]">
