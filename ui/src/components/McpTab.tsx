    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="panel-card lg:col-span-1 space-y-4">
        <h3 className="text-xs font-bold text-[#94a3b8] uppercase tracking-wider flex items-center gap-1.5">
          <Server size={14} className="text-indigo-400" /> MCP Servers Registry
        </h3>

        {isLoading ? (
          <p className="text-xs text-[#64748b] text-center py-8">Loading MCP servers...</p>
        ) : error ? (
            ...
            {servers.map((srv: any) => (
              ...
              >
                <div className="flex justify-between items-start">
                  <span className="font-bold text-indigo-400 font-mono truncate max-w-[120px]">{srv.server_key}</span>
                  {getStatusBadge(srv.status || "healthy")}
                </div>
                <div className="flex flex-col gap-1 mt-2 text-[10px] text-[#94a3b8] font-mono">
        ...
        {!selectedServerKey ? (
          ...
          <>
            <div className="border-b border-[#1f2937] pb-3 flex justify-between items-center">
              <h3 className="text-xs font-bold text-[#94a3b8] uppercase tracking-wider flex items-center gap-1.5">
                <History size={14} className="text-indigo-400" /> Manifest Drift History &middot; <code className="text-indigo-400 font-bold">{selectedServerKey}</code>
              </h3>
            </div>

            {isHistoryLoading ? (
              <p className="text-xs text-[#64748b] text-center py-8">Loading manifest history...</p>
            ) : !history || history.length === 0 ? (
              <p className="text-xs text-[#64748b] text-center py-8">No drift history logs available for this server.</p>
            ) : (
              <div className="space-y-3 max-h-[350px] overflow-y-auto custom-scrollbar">
                {history.map((record: any, idx: number) => (
                  <div key={idx} className="p-3 bg-[#0f172a]/40 border border-[#1f2937] rounded-lg text-xs flex justify-between items-start gap-4">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        <code className="text-indigo-400 font-mono text-[10px]">{record.manifest_hash ? record.manifest_hash.slice(0, 16) : "N/A"}</code>
                      </div>
                      <p className="text-[#94a3b8] text-[11px] font-sans mt-1">
