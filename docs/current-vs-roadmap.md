...
| Thread-Per-Core event fabric | `current` prototypes / `target` fabric | Isolated SPSC, safe sealed-page, append-only published-prefix, single-page volatile admission, and **bounded rotating admission** (ADR-0010: fixed `P`-page pool, exclusive in-place rebind, handoff ring, `PageQuotaExhausted`) prototypes exist under Proposed ADR-0006..0010. Composites validate before reservation, publish the page before the ring, withhold tail advancement until a validated frame lease commits, and make clean/faulted/orphan termination explicit. Tests: differential oracles, native stress, Loom, Miri-oriented lifetime cases, ASan/TSan CI lanes, zero-allocation checks; diagnostic criterion benches compile in CI; LLD §5 documents rotation linearization. Production carries no protected evidence, is neither `shadow` nor `qualified`, and has no published performance result. Blockers: formal ADR acceptance/security review, green hosted sanitizer artifacts, UBSan, authenticated registry, WAL durability/replay, NUMA-owner reclamation, priority lanes, production shadow wiring, release-artifact rollback, and qualification. |
