...
| Node sensor | Partial | process/net/**fs**/**secret** collectors + ProcessEnforcer for `AEGIS_RUN_ID` PIDs; Dockerfile, Helm, compose | richer attribution; UDP | Phased PR plan §5 | unit (kill + collectors) | beta | product UI |
...
3. ~~Sensor host enforce + process/net/fs/secret collectors~~ **Done** (AEGIS_RUN_ID-scoped; secret names/paths only)  
