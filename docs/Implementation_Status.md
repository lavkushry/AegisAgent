...
| Agent cage runner | Partial | forced egress: proxy env + **no-masquerade per-sandbox network** + wave-a e2e + exit_code | host-local residual; iptables REDIRECT | Phased PR plan §6 | unit + CI wave-a e2e | beta (local/k8s) | REDIRECT force |
| Egress proxy | Partial | binary + Helm + check API; cage no-masquerade net + forced proxy env | transparent REDIRECT / sidecar | Phased PR plan §7 | unit + proxy tests | beta | REDIRECT |
