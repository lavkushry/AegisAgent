...
### R4 — Network is “none” by default; proxy-forced path uses an internal, no-masquerade bridge with the proxy joined as a sidecar

Default remains `--network none` (no egress at all). When `network.egress_proxy_url` is set, the runner creates a dedicated per-sandbox Docker bridge with **`--internal`** (Docker never wires an external route for this network — the actual isolation primitive) plus `com.docker.network.bridge.enable_ip_masquerade=false` (defense in depth against the NAT return path) via `docker_cli::create_egress_network`, instead of the shared default `bridge` network. `HTTP(S)_PROXY`/`NO_PROXY` are force-injected and tenant-supplied overrides for those keys are stripped. The network is removed on `destroy` (`docker_runtime.rs`'s `networks` map).

**Corrected (previously wrong in this doc):** an `--internal` network has **no route to the host at all**, not just no internet — `host.docker.internal` does not resolve to anything reachable there (verified directly: a container on such a bridge gets "Network unreachable"). A host-run proxy process was therefore never actually reachable, in any environment, regardless of what interface it bound to. The only thing an `--internal` bridge *can* reach is another container joined to that same bridge, so the proxy must run as a container and be explicitly `docker network connect`-ed to each sandbox's dedicated bridge (`egress_proxy_container` config on `aegis-cage-runner`/`DockerRuntime`) — addressed by container name (Docker's embedded per-network DNS), not `host.docker.internal`. Without `egress_proxy_container` configured, a forced-egress sandbox gets no proxy connectivity at all — fail closed.

**Residual:** `--cap-drop ALL` already removes `CAP_NET_RAW`, so raw sockets are not the relevant residual vector (an earlier draft of this doc said so — corrected here). The residual vector is an ordinary (non-raw) UDP/TCP socket reaching anything still routable from the internal bridge — by design that's only the joined proxy container, so this is expected to be a dead end for the sandbox, but it depends on `--internal` behaving as documented on the CI/production Docker version in use; this is exercised by `scripts/cage-wave-a-e2e.sh`'s forced-egress phase (`wget` to the public Internet must fail) and by `docker_runtime::forced_egress_sandbox_reaches_the_sidecar_proxy_container` (asserts the sandbox *can* reach the joined proxy). `allowed_destinations` enforcement itself still lives entirely in the proxy (`aegis-egress-proxy`), not in this Docker-level control.

**Residual:** this is not a full netns solution — a cooperative HTTP client is required to honor `HTTP_PROXY` (a raw socket that ignores it and only targets the joined proxy container is still possible, though it can no longer NAT out to the public Internet without masquerade, and the proxy itself is the only thing reachable at all). `allowed_destinations` enforcement lives entirely in the proxy (`aegis-egress-proxy`), not in this Docker-level control — a compromised proxy or a destination it explicitly allows is out of scope for this control.
